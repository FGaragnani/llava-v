...
def preprocess_v1(
    ...
    # Mask targets using exact token counts
    token_count_cache = {}
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for inst in instances:
            if inst.get('is_grand', False):
                any_grand = True
                grand_bboxes.append(inst.get('bboxes', []))
                grand_image_paths.append(inst.get('image_path', None))
                grand_dense_captions.append(inst.get('dense_caption', ""))
                grand_dense_labels.append(inst.get('dense_labels', []))
            else:
                grand_bboxes.append([])
                grand_image_paths.append(None)
                grand_dense_captions.append("")
                grand_dense_labels.append([])
        if any_grand:
            batch['grand_bboxes'] = grand_bboxes
            batch['grand_image_paths'] = grand_image_paths
            batch['grand_dense_captions'] = grand_dense_captions
            batch['grand_dense_labels'] = grand_dense_labels
        return batch
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    # Optional GranDDataset (GLAMM) integration
    grand_dataset = None
    if getattr(data_args, "use_glamm", False) and data_args.grand_image_dir and data_args.grand_annotation_dir:
        ...
        if GranDDataset is not None:
            grand_dataset = GranDDataset(
                ...
                prompt_template=data_args.glamm_instruction_label,
            )
    ...
    if train_dataset and grand_dataset:
        combined = ConcatDataset([train_dataset, grand_dataset])
    else:
        combined = train_dataset or grand_dataset
    if combined is None:
        raise ValueError("No training dataset available: provide --data_path or enable --use_glamm with directories.")
