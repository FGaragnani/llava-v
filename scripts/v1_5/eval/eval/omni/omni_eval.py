...
if IS_LLAVA_MORE:
    ...
    print("Using imports from CAMBRIAN")
...
def process(line, args, tokenizer, image_processor, model_config, img_path):
    qs = line["prompt"]
...
def eval_model(args):
    ...
    questions = []
    hf_root = Path(os.getenv('HF_DATASETS_CACHE', os.path.expanduser('~/.cache/huggingface/datasets'))).joinpath('nyu-visionx___cv-bench')
    with open(hf_root.joinpath(args.prompt_path), 'r') as file:
        questions = [json.loads(line.rstrip()) for line in file.readlines()]

    img_root = hf_root.joinpath(args.images_path)
    image_paths = [str(img_root.joinpath(str(i).zfill(6))) + '.png' for i in range(len(questions))]
    ...
    print(valid_chunk)
    for line, img_path in tqdm(zip(questions, image_paths), total=len(questions)):
        idx = idx+1
        if idx<valid_chunk[0] or idx>valid_chunk[1]:
            continue
    
        input_ids, image_tensor, image_sizes, prompt = process(line, args, tokenizer, image_processor, model.config, img_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--images_path", type=str, default="img/3D")
    parser.add_argument("--prompt_path", type=str, default="test_3d.jsonl")
